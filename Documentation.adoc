...
|filterMode |String |  |Mode to filter data within the column. It is defaulted to 'equals'.Current filter modes supported by the Grid component are listed as enum values
...
Notice *@Repo(value=Database.rep_ws)* in the code above. This indicates that it will make use of an external web service call rahter than
a MongoDb call for ExtClient data objects' persistence/ retrieval purposes.

The advantage of this feature is that now we do not rely on internal source only. This gives us better flexibility and maintainabil
...
As you can see, when you click on addButton, the config url will follow the domain model path until the form view where it can display
and then switch from view config to core config using .m and it will assign a new car to attr_list_2_CarNestedEntity list. 
There is also a List that points to the nested entity, hence it will display a list of cars that are created. 
The Section_ConvertedNestedEntity class has an edit button that will take you to the appropriate page. The edit button config url is resolved 
in a similar way to that of add button, however, as you can see, there is a .d that signifies the mapping to the core domain model i.e. it will point to the
root which is CarEntity. There is also a variable vm_attached_convertedList, that points to car_name i.e. name of the car. 
...
delete functions respectively, hence eliminating the use of functional handlers.
