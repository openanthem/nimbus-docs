...
| context	| ParamContext | @ParamContext(enabled = false, visible = false) | N | Specify the default contextual properties.
...
In this scenario we have configured the contextual values for _enabled_ and _visible_ to be **true** for _myModal_. These values will 
be set during the _OnStateLoad_ event.
